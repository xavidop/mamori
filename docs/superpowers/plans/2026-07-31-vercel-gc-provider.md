...
// roundTripper drives the handler in-process, with no listener and no
// background goroutine.
//
// This is why no test here starts a real httptest.Server: providertest's
// NoGoroutineLeak case runs goleak.VerifyNone with no ignore options, and a
// live server's accept goroutine can never satisfy that.
