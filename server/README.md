...
- A binding ref may not carry **`?decode=`**: this server serves what the upstream holds and never runs the decode pipeline, so accepting the option would silently serve still-encoded bytes. It is rejected at construction; put it on the consuming client's own ref (`source:"mamori://tls-key?decode=base64"`), where core applies it. `#key` fragments, pointers included, are unaffected - providers apply those themselves.
