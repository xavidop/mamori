...
- **Forceable** - `w.Refresh(ctx)` re-resolves every field right now, bypassing poll intervals and backoff, and blocks until the result is applied or rejected (through the same `PreApply` gate, never bypassing it), so a SIGHUP handler or your own admin route learns whether the reload actually worked.
