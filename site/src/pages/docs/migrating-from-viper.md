...
This page does not make the case for switching; for that, see [Why mamori](/docs/comparison/). Here the point is narrower and more concrete: adoption is incremental, field by field, at whatever pace fits, and some fields may reasonably never move.
...
`DBPassword` no longer touches Viper at all; `aws-sm://` works the same way if that is your secret manager instead. That one field change buys three things immediately, none of which a `viper://` ref could ever provide. First, redaction everywhere the value would normally be rendered, in a log line, `fmt`, or a JSON dump (see [Secret types](/docs/concepts/secret-types/)). Second, live rotation, so a credential changed in the backend reaches the running process with no restart. Third, the `PreApply` gate, so a rotated credential is proven to actually work before it becomes the config your application serves (see [Rotation safety](/docs/usage/rotation/)). The remaining fields keep resolving through Viper exactly as before.
...
`Port` now prefers a plain `env:` ref and only falls through to `viper://server.port` if `PORT` is unset, so the cutover is reversible right up until you delete the fallback. `LogLevel` is left untouched. A field like a log level, read once and rarely changed, may never need to move at all, and leaving it on `viper://` is not a compromise, it is the honest outcome for a field with no rotation, no secrecy, and no validation need beyond what Viper already gives it.
...
**`viper.WatchConfig()` becomes unsafe on an instance mamori is also polling, because Viper is not safe for concurrent read and write.** Once a `viper://` ref points at that instance, stop calling `viper.WatchConfig()` on it and let mamori's own poller detect changes instead. If you want native file-change detection without polling, use mamori's built-in [`file://`](/docs/providers/file/) provider. See [Viper provider: concurrency](/docs/providers/viper/#concurrency-do-not-mutate-a-viper-instance-mamori-is-polling) for the full race and why it happens.

Neither gap is fatal to the migration. Both just mean the fields that depend on runtime mutation or `WatchConfig()` are exactly the fields to migrate off `viper://` earliest, or to consciously decide to leave them on it.
