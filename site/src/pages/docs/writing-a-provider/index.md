...
- **Polling watches, and postgres's native `LISTEN`/`NOTIFY` watch, degrade to an error stream.** `pollWatch`'s loop resolves on every tick through the same closed-gated accessor `Resolve` uses, so a post-`Close` tick reports `mamori.ErrUnavailable`, emits it as `Update{Err: err}`, and the loop keeps polling and keeps erroring; postgres's watch loop resolves each `LISTEN` notification the same way. Either way the watch stays alive and visibly failing until its own context is cancelled.
- **etcd and redis are worse than an error stream.** `Close` tears down the client the watch is already reading from: closing the etcd client closes every open watch channel, and closing the go-redis client's connection pool ends its subscription's channel. Both providers' watch loops treat a closed channel as a plain, silent return, with no error emitted - and mamori's own reconciler forwarder does the same with the resulting closed `Update` channel: it returns without ever calling `OnError`. So a watch on etcd or redis that is already running when `Close` runs goes silently dead: `Get()` keeps serving the stale value, and nothing distinguishes "closed out from under you" from "quietly healthy" until you separately cancel that watch's own context.
