...
`Close` does not stop a `Watch` that is already running. The SSE connection it is streaming on is not one of the idle connections `Close` releases, so it keeps delivering live updates until it next reconnects; from there it becomes an error stream carrying `errors.Is(err, mamori.ErrUnavailable)`, retried with backoff. Cancel the watch's own context to stop it. [Close does not stop a Watch](/docs/writing-a-provider/#close-does-not-stop-a-watch) compares every provider.
