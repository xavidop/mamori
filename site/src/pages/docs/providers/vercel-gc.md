...
`GLOBAL_CONFIG` (falling back to `EDGE_CONFIG` for stores connected before Vercel's rename) is read lazily at resolve time, so a blank import alone is enough once the environment variable is set. The token always travels as an `Authorization: Bearer` header, never a URL query parameter, so it cannot leak into a log line, a trace span, or an error message.
