...
// through fakeSupabase's in-process RoundTripper rather than a real
...
func TestConformance(t *testing.T) {
	...
	providertest.Run(t, providertest.Config{
		...
		Ref: func(key string) string { return "supabase://" + key },
		...
		// A Vault secret's value may itself be JSON, and #key selects into it
		// through mamori.SelectKey exactly as in every other provider, so this
		// case applies and must run.
		PointerRef: func(key, frag string) string {
			return "supabase://" + key + frag
		},
	})
}
