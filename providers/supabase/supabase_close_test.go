...
//
// A real httptest.Server is used rather than fakeSupabase's in-process
// RoundTripper: the first assertion needs an actual round trip over the
// client's real transport, which an in-process fake has none of.
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	...
	p := New(WithProjectURL(srv.URL), WithServiceKey(testServiceKey), WithHTTPClient(client), WithAllowInsecure())
