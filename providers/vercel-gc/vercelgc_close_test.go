...
func vercelgcHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	if strings.HasSuffix(r.URL.Path, "/digest") {
		_, _ = w.Write([]byte(`"digest-1"`))
		return
	}
...
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	srv := httptest.NewTLSServer(http.HandlerFunc(vercelgcHandler))
	t.Cleanup(srv.Close)
	client := srv.Client()

	p := New(WithStoreID("store"), WithToken("test-token"), WithBaseURL(srv.URL), WithHTTPClient(client))
	testRef := ref(t, "vercel-gc://K")

	if _, err := p.Resolve(context.Background(), testRef); err != nil {
	...
	if _, err := p.Resolve(context.Background(), testRef); !errors.Is(err, mamori.ErrUnavailable) {
...
//
// A service that resolves values from Global Config and also calls some
