...
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	...
	p := New(
		...
		WithNamespaceID("test-namespace"),
		WithHTTPClient(client),
	)
	testRef := ref(t, "cloudflare-kv://K")
...
//
// A service that resolves values from Workers KV and also calls some other
// API through anything built on http.DefaultClient (very common: any
