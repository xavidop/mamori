...
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"flags":{"new-checkout":{"key":"new-checkout","enabled":true}}}`))
	}))
	t.Cleanup(srv.Close)
	client := srv.Client()

	p := New(WithHost(srv.URL), WithProjectAPIKey("test-key"), WithHTTPClient(client))
	ref := mustRef(t, "posthog://new-checkout")
