...
// through to http.DefaultTransport, which does not trust that certificate, so
// the request would genuinely fail instead of quietly succeeding against
...
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	...
	p := New(WithBaseURL(srv.URL), WithToken(testToken), WithHTTPClient(client))
	ref := mustRef(t, "doppler://"+testProject+"/"+testConfig+"#K")
...
//
// A service that resolves secrets from Doppler and also calls some other API
// through anything built on http.DefaultClient (very common: any library that
// omits a Transport) shares that pool whether it knows it or not. If Close
// called CloseIdleConnections unconditionally, closing this provider on every
// config reload would silently evict that unrelated client's idle
