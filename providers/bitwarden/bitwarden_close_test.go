...
//
// A real httptest.Server is used rather than fakeBackend's in-process
// RoundTripper: the first assertion needs an actual round trip over the
// client's real transport, which an in-process fake has none of. Both the
// identity and API endpoints are served from the same server, routed by
// path, since fakeBackend's serveIdentity/serveAPI only look at the request
// path, not the host the in-process transport otherwise switches on.
