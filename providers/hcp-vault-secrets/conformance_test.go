...
// through fakeHCP's in-process RoundTripper rather than a real httptest.Server:
// the NoGoroutineLeak case runs goleak.VerifyNone with no ignore options, which
// a live server's accept goroutine could never satisfy.
//
...
func TestConformance(t *testing.T) {
	...
	providertest.Run(t, providertest.Config{
		...
		Ref: func(key string) string { return "hcp-vs://" + key },
		...
		//
		// Only the read path is failed. Failing the token leg too would route
		// the error through httpcore's authError, which adds
		// ErrUnauthenticated to an unclassified cause, so the kit would see one
		// kind for every case. A token that is valid but refused one particular
		// secret is also the real shape of a per-secret failure on HCP, where
		// authorization is per application.
