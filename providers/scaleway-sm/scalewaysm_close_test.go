...
func TestCloseLeavesInjectedClientUsable(t *testing.T) {
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"secret_id":"s1","revision":1,"data":"dg==","type":"opaque"}`))
	}))
	t.Cleanup(srv.Close)
	client := srv.Client()
...
//
// A service that resolves secrets from Secret Manager and also calls some
